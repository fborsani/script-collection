...
foldername="dump_$(hostname)_$(date '+%Y%m%d_%H%M%S')"
mkdir $foldername
...
tar -czvf $foldername/tomcat_logs.tar.gz "$tomcat"

echo "[*] Retrieving application logs..."
for d in $(find / -type d -name WEB-INF); do file="$(echo ${d%/*} | grep -oG /[^/]*$ | cut -c 2-).tar.gz"; tar -czvf "$foldername/$file"  "$d/log"; done

echo "[*] Dumping threads..."
for i in $(seq $count); do echo "Dumping threads $i of $count times...";jcmd $pid Thread.print > ./$foldername/threads_$i.txt;sleep $sleep_time; done
...
rm -rf ./$foldername
echo "[+] Done!"
echo "[+] archive is at $(pwd)/$foldername.tar.gz"
